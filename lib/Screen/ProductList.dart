...
import 'package:ziberto_vendor/NewScreen/edit_product.dart';
...
class StateProduct extends State<ProductList> with TickerProviderStateMixin {
  ...
  Widget listItem(int index) {
    if (index < productList.length) {
      ...
      items = new List<String>.generate(
          model.totalAllow != ""&&model.totalAllow!=null ? int.parse(model.totalAllow.toString()) : 10,
          (i) => (i + 1).toString());
                        ...
                        children: <Widget>[
                          Row(
                            ...
                            children: [
                              ...
                              model.product_status!=null?
                              Switch(
                                  value: model.product_status=="1"?true:false,
                                  onChanged: (val){
                                    if(val){
                                      setState(() {
                                        model.product_status="1";
                                      });
                                      updateProductList(model.id, "1");
                                    }else{
                                      setState(() {
                                        model.product_status="0";
                                      });
                                      updateProductList(model.id, "0");
                                    }
                              }
                              ):SizedBox(),
                            ],
                            ...
                            children: [
                              Row(
                                children: [
                                  ...
                                  InkWell(
                                    onTap: () {
                                      Navigator.push(context, MaterialPageRoute(builder: (context)=> EditProduct(productList1[index])));
                                    },
                                    child: Card(
                                      child: Icon(
                                        Icons.edit_outlined,
  ...
  updateProductList(id,status) async{
    _isNetworkAvail = await isNetworkAvailable();
    if(_isNetworkAvail){
      var param = {
        "status":status,
      ...
      apiBaseHelper.postAPICall(updateProductsApi, param).then((value){
        ...
        Container(
          ...
          child: Material(
            ...
            child: PopupMenuButton(
              color: AppColor().colorBg1(),
              icon: Icon(Icons.more_vert,color: AppColor().colorBg1(),),
